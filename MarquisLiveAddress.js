...
(function ($, window, document) {
    ...
    var defaults = {
        ...
        requestStreetIntl: "https://international-street.api.smartystreets.com/verify", // International API endpoint
    ...
    $.fn.LiveAddress = function (arg) {
        ...
        if (vers.length >= 2) {
            if (vers[0] < 1 || (vers[0] == 1 && vers[1] < 5)) {
                console.log(
                    "jQuery version " +
                        $.fn.jquery +
                        " found, but LiveAddress requires jQuery version 1.5 or higher. Aborting."
                );
                return false;
            }
        } else {
        ...
        if (arg.debug) console.log("LiveAddress API jQuery Plugin version " + version + " (Debug mode)");
        ...
        /**(Plugin Config) The number of matches to return when the address is ambiguous. 
		If you set this to 1, all ambiguous addresses will appear to be 
		perfectly valid instead.*/
        config.candidates = config.candidates || defaults.candidates;

        /**(Plugin Config) Whether to provide and execute the default UI for handling the address verification 
		process, including autocomplete. Disable this to use your own. Only developers with 
		experience using this plugin should disable its UI. */
        config.ui = typeof config.ui === "undefined" ? true : config.ui;

        /**(Plugin Config) Verify the address as soon as enough of it has been typed by the user. 
		This will only happen once per address before the form is submitted to avoid annoying the user.*/
        config.autoVerify = config.autoVerify !== true && config.autoVerify !== false ? true : config.autoVerify;

        /**(Plugin Config) Verify addresses in a form when the form is submitted. 
		If you set both this and autoVerify to false, no validation will occur automatically at all.*/
        config.submitVerify = typeof config.submitVerify === "undefined" ? true : config.submitVerify;

        /**(Plugin Config) Milliseconds before an API request is considered to have failed if no response is received.*/
        config.timeout = config.timeout || defaults.timeout;

        /**(Plugin Config) The message to display when an address is ambiguous. Keep it short.*/
        config.ambiguousMessage = config.ambiguousMessage || defaults.ambiguousMessage;

        /**(Plugin Config) The message to display when an address is invalid. Keep it short.*/
        config.invalidMessage = config.invalidMessage || defaults.invalidMessage;

        /**(Plugin Config) The message to display when a country is invalid. Keep it short.*/
        config.invalidCountryMessage = config.invalidCountryMessage || defaults.invalidCountryMessage;

        /**(Plugin Config) The message to display when an address is missing a valid secondary number when verifySecondary is set to true.*/
        config.missingSecondaryMessage = config.missingSecondaryMessage || defaults.missingSecondaryMessage;

        /**(Plugin Config) The message to display where the user clicks to bypass verification and certify the address is correct.*/
        config.certifyMessage = config.certifyMessage || defaults.certifyMessage;

        /**(Plugin Config) The message to display when an address is missing input in order to verify.*/
        config.missingInputMessage = config.missingInputMessage || defaults.missingInputMessage;

        /**(Plugin Config) The message to display where the user clicks to go back to the form and edit their address.*/
        config.changeMessage = config.changeMessage || defaults.changeMessage;

        /**(Plugin Config) The message to display when there are no autocomplete suggestions available for the provided user input.*/
        config.noSuggestionsMessage = config.noSuggestionsMessage || defaults.noSuggestionsMessage;

        /**(Plugin Config) The CSS selector which identifies input fields inside each form. 
		Do not attempt to use this to map the form fields.*/
        config.fieldSelector = config.fieldSelector || defaults.fieldSelector;

        /**(Plugin Config) The CSS selector which identifies the submit button for your form. 
		You don't usually need to set this yourself, but if you do, it's easiest to select by the button's ID. */
		...
		Can be used without filtering enabled as well. */
        config.waitForStreet = typeof config.waitForStreet === "undefined" ? false : config.waitForStreet;

        /**(Plugin Config) If addresses are missing secondary numbers (e.g., apartment number, suite number) 
		then the plugin will display the missingSecondaryMessage and allow the user to go back and change 
		their address or accept the address as is. */
        config.verifySecondary = typeof config.verifySecondary === "undefined" ? false : config.verifySecondary;

        /**(Plugin Config) Whether or not to request geocoded addresses from the international API.*/
        config.geocode = typeof config.geocode === "undefined" ? false : config.geocode;

        /**(Plugin Config) Removes the option for the user to "certify the address is correct" and bypass verification. 
		This makes the form strict enough to disallow anyone from submitting the form without entering a valid address.*/
        config.enforceVerification =
            typeof config.enforceVerification === "undefined" ? false : config.enforceVerification;
        ...
        /**(Service Input Field) The number of suggestions to show while the user types in the street field. 
		Set to 0 to disable autocomplete. Previously 'autocomplete.'*/
        config.max_results = typeof config.max_results === "undefined" ? defaults.max_results : config.max_results;

        /**(Service Input Field) Limit the results to only those cities and states listed, as well as those in include_only_states. 
		Previously 'cityFilter' (5.2), 'cityStatePreference' (2.8). Semi-colon delimited (e.g., DENVER,AURORA,CO;OMAHA,NE)*/
        config.include_only_cities =
        ...
        instance = {
            ...
            events: {
                FieldsMapped: function (event, data) {
                    if (config.debug)
                        console.log(
                            "EVENT:",
                            "FieldsMapped",
                            "(Fields mapped to their respective addresses)",
                            event,
                            data
                        );
                ...
                MapInitialized: function (event, data) {
                    if (config.debug)
                        console.log(
                            "EVENT:",
                            "MapInitialized",
                            "(Mapped fields have been wired up to the window" +
                                (config.ui ? ", document, and UI" : " and document") +
                                ")",
                            event,
                            data
                        );
                ...
                AutocompleteReceived: function (event, data) {
                    if (config.debug)
                        console.log(
                            ...
                            "(A response has just been received from the autocomplete service)",
                            event,
                            data
                        );
                ...
                AutocompleteUsed: function (event, data) {
                    if (config.debug)
                        console.log(
                            "EVENT:",
                            "AutocompleteUsed",
                            "(A suggested address was used from the autocomplete service)",
                            event,
                            data
                        );
                },
                ...
                AddressChanged: function (event, data) {
                    if (config.debug) console.log("EVENT:", "AddressChanged", "(Address changed)", event, data);
                    /**
					...
					*/
                    ...
                    // THEN verification has been invoked.
                        ...
                        config.autoVerify &&
                        data.address.enoughInput() &&
                        (data.address.verifyCount == 0 || data.address.isFreeform() || data.address.usedAutocomplete) &&
                        !data.suppressAutoVerification &&
                        data.address.hasDomFields() &&
                        data.address.active &&
                        !data.address.autocompleteVisible() &&
                        data.address.form &&
                        !data.address.form.processing
                    )
                ...
                VerificationInvoked: function (event, data) {
                    ...
                    if (!data.address || (data.address && data.address.form && data.address.form.processing)) {
                        if (config.debug)
                            console.log(
                                "NOTICE: VerificationInvoked event handling aborted. Address is missing or an address in the " +
                        ...
                        if (config.debug)
                            console.log(
                                "NOTICE: VerificationInvoked raised on an accepted or un-changed address. Nothing to do."
                            );
                        return trigger("Completed", data);
                    } else if (data.address.form) data.address.form.processing = true;
                ...
                ResponseReceived: function (event, data) {
                    if (config.debug)
                        console.log(
                            "EVENT:",
                            "ResponseReceived",
                            "(Response received from server, but has not been inspected)",
                            event,
                            data
                        );
                    ...
                    if (typeof data.invoke === "function") data.invoke(data.response); // User-defined callback function; we're all done here.

                    if (data.response.isInvalid()) trigger("AddressWasInvalid", data);
                    else if (data.response.isAmbiguous()) trigger("AddressWasAmbiguous", data);
                    else if (config.verifySecondary && data.response.isMissingSecondary())
                        trigger("AddressWasMissingSecondary", data);
                    else if (data.response.isValid()) trigger("AddressWasValid", data);
                },

                RequestTimedOut: function (event, data) {
                    if (config.debug) console.log("EVENT:", "RequestTimedOut", "(Request timed out)", event, data);

                    if (data.address.form) delete data.address.form.processing; // Tell the potentially duplicate event handlers that we're done.

                    // If this was a form submit, don't let a network failure hold them back; just accept it and move on
                    if (data.invoke) data.address.accept(data, false);
                ...
                AddressWasValid: function (event, data) {
                    if (config.debug)
                        console.log(
                            "EVENT:",
                            "AddressWasValid",
                            "(Response indicates input address was valid)",
                            event,
                            data
                        );
                ...
                AddressWasAmbiguous: function (event, data) {
                    if (config.debug)
                        console.log(
                            "EVENT:",
                            "AddressWasAmbiguous",
                            "(Response indiciates input address was ambiguous)",
                            event,
                            data
                        );
                ...
                AddressWasInvalid: function (event, data) {
                    if (config.debug)
                        console.log(
                            "EVENT:",
                            "AddressWasInvalid",
                            "(Response indicates input address was invalid)",
                            event,
                            data
                        );
                ...
                CountryWasInvalid: function (event, data) {
                    if (config.debug)
                        console.log(
                            "EVENT:",
                            "CountryWasInvalid",
                            "(Pre-verification check indicates that the country was invalid)",
                            event,
                            data
                        );
                ...
                AddressWasMissingSecondary: function (event, data) {
                    if (config.debug)
                        console.log(
                            ...
                            "(Response indicates input address was missing secondary",
                            event,
                            data
                        );
                ...
                AddressWasMissingInput: function (event, data) {
                    if (config.debug)
                        console.log(
                            ...
                            "(Pre-verification check indicates that there was not enough input)",
                            event,
                            data
                        );
                ...
                AddedSecondary: function (event, data) {
                    if (config.debug)
                        console.log(
                            "EVENT:",
                            "AddedSecondary",
                            "(User entered a secondary number to attempt revalidation)",
                            event,
                            data
                        );
                    data.address.verify(data.invoke, data.invokeFn);
                ...
                UsedSuggestedAddress: function (event, data) {
                    if (config.debug)
                        console.log(
                            ...
                            "(User chose to a suggested address)",
                            event,
                            data
                        );
                ...
                InvalidAddressRejected: function (event, data) {
                    if (config.debug)
                        console.log(
                            ...
                            "(User chose to correct an invalid address)",
                            event,
                            data
                        );

                    if (data.address.form) delete data.address.form.processing; // We're done with this address and ready for the next, potentially
                ...
                AddressAccepted: function (event, data) {
                    ...
                    if (data.address && data.address.form) delete data.address.form.processing; // We're done with this address and ready for the next, potentially

                    // If this was the result of a form submit, re-submit the form (whether by clicking the button or raising form submit event)
                    if (data.invoke && data.invokeFn) submitForm(data.invoke, data.invokeFn);
            ...
            on: function (eventType, userHandler) {
                if (!this.events[eventType] || typeof userHandler !== "function") return false;
            ...
            mapFields: function (map) {
                var doMap = function (map) {
                    if (typeof map === "object") return ui.mapFields(map);
                    else if (!map && typeof config.addresses === "object") return ui.mapFields(config.addresses);
                    else return false;
                };
                if ($.isReady) doMap(map);
                else
                    $(function () {
                        doMap(map);
                    });
            ...
            makeAddress: function (addressData) {
                if (typeof addressData !== "object")
                    return (
                        instance.getMappedAddressByID(addressData) ||
                        new Address({
                            freeform: addressData,
                        })
                    );
                else return new Address(addressData);
            },
            ...
            getMappedAddresses: function () {
                ...
                for (var i = 0; i < forms.length; i++)
                    for (var j = 0; j < forms[i].addresses.length; j++) addr.push(forms[i].addresses[j]);
                return addr;
            },
            getMappedAddressByID: function (addressID) {
                for (var i = 0; i < forms.length; i++)
                    for (var j = 0; j < forms[i].addresses.length; j++)
                        if (forms[i].addresses[j].id() == addressID) return forms[i].addresses[j];
            },
            setKey: function (htmlkey) {
                config.key = htmlkey;
            },
            setIncludeOnlyCities: function (cities) {
                config.include_only_cities = cities;
            },
            setIncludeOnlyStates: function (states) {
                config.include_only_states = states;
            },
            setIncludeOnlyZipCodes: function (zips) {
                config.include_only_zip_codes = zips;
            },
            setPreferCities: function (cities) {
                config.prefer_cities = cities;
            },
            setPreferStates: function (states) {
                config.prefer_states = states;
            },
            setPreferZipCodes: function (zips) {
                config.prefer_zip_codes = zips;
            },
            setExcludeStates: function (states) {
                config.exclude_states = states;
            },
            ...
            autoVerify: function (setting) {
                if (typeof setting === "undefined") return config.autoVerify;
                else if (setting === false) config.autoVerify = false;
                else if (setting === true) config.autoVerify = true;
                for (var i = 0; i < forms.length; i++) {
                    for (var j = 0; j < forms[i].addresses.length; j++) {
    ...
    function UI() {
        ...
        var uiCss =
            "<style>" +
            ".smarty-dots { display: none; position: absolute; z-index: 999; width: " +
            loaderWidth +
            "px; height: " +
            loaderHeight +
            "px; " +
            ...
            ".smarty-popup-ambiguous-header { color: #CEA737; }" +
            ".smarty-popup-invalid-header { color: #D0021B; }" +
            ...
            " padding: 2px 15px; font-size: 11pt; width: 215px; }" +
            "#smarty-popup-secondary-number-form-submit-button:hover { background: #333; }" +
            ...
            ".smarty-choice-list .smarty-choice:hover { color: #333; " +
            "background: #F7F7F7; text-decoration: none; border: 1px solid #333 }" +
            ".smarty-choice-alt { " +
            "background: inherit; clear: both; }" +
            ".smarty-choice-alt" +
            ...
            " .smarty-choice-abort { float: left }" +
            ".smarty-choice-alt " +
            ".smarty-choice:first-child { border-top: 0; }" +
            ".smarty-choice-abort:hover { background: #333; }" +
            ".smarty-choice-override:hover { background: #333; }" +
            ".smarty-tag { position: absolute; " +
            ...
            ".smarty-tag:hover { width: 70px; " +
            "text-decoration: none; color: #999; }" +
            ".smarty-tag:hover .smarty-tag-text " +
            "{ color: #000; }" +
            ".smarty-tag-grayed { border: 1px solid #B4B4B4; " +
            "color: #999; background: #DDD; box-shadow: inset 0 9px 15px #FFF; }" +
            ".smarty-tag-green { border: 1px solid #407513; color: #407513; " +
            "background: #A6D187; box-shadow: inset 0 9px 15px #E3F6D5; }" +
            ".smarty-tag-grayed:hover " +
            "{ border-color: #333; }" +
            ".smarty-tag-check { padding-left: 4px; " +
            "text-decoration: none; }" +
            ".smarty-tag-text { font-size: 12px; position: absolute; " +
            "top: 0; left: 16px; width: 50px; text-align: center; }" +
            ".smarty-autocomplete " +
            "{ border: 1px solid #777; background: white; overflow: hidden; white-space: nowrap; " +
            "box-shadow: 1px 1px 3px #555; }" +
            ".smarty-suggestion { display: block; color: #444; " +
            "text-decoration: none; font-size: 12px; padding: 1px 5px; }" +
            ".smarty-active-suggestion " +
            "{ background: #EEE; color: #000; border: none; outline: none; }" +
            ...
            ".smarty-debug-button { color: #4ba341 !important; } " +
            "</style>";
        ...
        this.postMappingOperations = function () {
            ...
            if (config.ui) {
                ...
                for (var i = 0; i < addresses.length; i++) {
                    ...
                    $("body").append(
                        '<div class="smarty-ui"><div title="Loading..." class="smarty-dots smarty-addr-' +
                            id +
                            '"></div></div>'
                    );
                    if (config.smartyTag) {
                        ...
                        $("body").append(
                            '<div class="smarty-ui" style="top: ' +
                                offset.top +
                                "px; left: " +
                                offset.left +
                                'px;"><a href="javascript:void(0)" class="smarty-tag smarty-tag-grayed smarty-addr-' +
                                id +
                                '" title="Address not verified. Click to verify." data-addressid="' +
                                id +
                    ...
                    $(window).on(
                        ...
                        function (e) {
                            ...
                            if (config.max_results) {
                                // Position of autocomplete boxes
                                var containerUi = $(".smarty-autocomplete.smarty-addr-" + addr.id()).closest(
                                    ".smarty-ui"
                                );
                                ...
                                if (mainInput !== "") {
                                    containerUi.css({
                                        ...
                                        top:
                                            $(domFields[mainInput]).offset().top +
                                            $(domFields[mainInput]).outerHeight(false) +
                                            "px",
                                    });
                                }
                            }
                ...
                if (config.max_results && config.key) {
                            ...
                            if (mainInput !== "") {
                                ...
                                strField.keydown(
                                    ...
                                    function (event) {
                                        ...
                                        var currentChoice = $(
                                            ".smarty-active-suggestion:visible",
                                            suggContainer
                                        ).first();
                                        ...
                                        if (event.keyCode == 9) {
                                            ...
                                            if (currentChoice.length > 0) {
                                                ...
                                                if (domFields.zipcode) $(domFields.zipcode).focus();
                                                else $(domFields[mainInput]).blur();
                                                useAutocompleteSuggestion(
                                                    event.data.addr,
                                                    autocompleteResponse.suggestions[currentChoice.data("suggIndex")],
                                                    event.data.containerUi
                                                );
                                                return addr.isFreeform() ? true : suppress(event);
                                            } else ui.hideAutocomplete(event.data.addr.id());
                                        } else if (event.keyCode == 40) {
                                            // Down arrow
                                            if (!currentChoice.hasClass("smarty-suggestion")) {
                                                currentChoice = $(".smarty-suggestion", suggContainer)
                                                    .first()
                                                    .mouseover();
                                            ...
                                            if (!choiceSelectionIsNew) {
                                                if (
                                                    currentChoice.next(
                                                        ".smarty-addr-" + event.data.addr.id() + " .smarty-suggestion"
                                                    ).length > 0
                                                )
                                                    currentChoice.next(".smarty-suggestion").mouseover();
                                                else currentChoice.removeClass("smarty-active-suggestion");
                                            }
            ...
            if (config.submitVerify) {
                ...
                for (var i = 0; i < forms.length; i++) {
                    ...
                    submitHandler = function (e) {
                        ...
                        if (
                            (e.data.form && e.data.form.processing) ||
                            $(".smarty-active-suggestion:visible").length > 0
                        )
                            return suppress(e);
			...
			*/
                    ...
                    var bindSubmitHandler = function (domElement, eventName) {
                        if (!domElement || !eventName) return;
        ...
        function doAutocomplete(event) {
                ...
                if ($(".smarty-active-suggestion:visible").length > 0)
                    useAutocompleteSuggestion(
                        ...
                        autocompleteResponse.suggestions[
                            $(".smarty-active-suggestion:visible").first().data("suggIndex")
                        ],
                        containerUi
                    );
                ui.hideAutocomplete(addr.id());
            ...
            if (!input || input == addr.lastStreetInput || !addr.isDomestic()) return;
        ...
        function getSearchValFromEntriesSuggestion(suggestionText) {
            ...
            if (suggestionText && typeof suggestionText !== "undefined") {
                searchVal = suggestionText.replace(/(.*)\(([\w\d]*)\sentries\)\s(.*)/gi, "$1").trim();
            }
            return searchVal;
        }
        ...
        function buildSecondarySearchParms(addressStr) {
            ...
            if (addressStr && typeof addressStr !== "undefined") {
                ...
                let entries = addressStr.replace(/(.*)\(([\w\d]*)\sentries\)\s(.*)/gi, "$2").trim();
                let addressEndMatter = addressStr.replace(/(.*)\(([\w\d]*)\sentries\)\s(.*)/gi, "$3").trim();
                parms["search"] = search;
        ...
        this.requestAutocomplete = function (event, data) {
            ...
            var autocplrequest = {
                callback: function (counter, json) {
                    ...
                    if (typeof data.suggContainer === "undefined")
                        data.suggContainer = $(".smarty-autocomplete", data.containerUi);
                    data.suggContainer.empty();

                    if (!json.suggestions || json.suggestions.length == 0) {
                        data.suggContainer.html(
                            '<div class="smarty-no-suggestions">' + config.noSuggestionsMessage + "</div>"
                        );
                        return;
                    }
                    ...
                    if (config.waitForStreet && filtering == false) {
                        var message = "";
                        if (config.include_only_states || config.include_only_cities || config.include_only_zip_codes) {
                            message = "filtered";
                        } else {
                            message = "address";
                        }
                        data.suggContainer.html(
                            '<div class="smarty-no-suggestions">Type more for ' + message + " suggestions</div>"
                        );
                    } else {
                        for (var j = 0; j < json.suggestions.length; j++) {
                            ...
                            suggAddr = suggAddr.replace(
                                new RegExp("(" + data.input.replace(/[#-.]|[[-^]|[?|{}]/g, "\\$&") + ")", "ig"),
                                "<b>$1</b>"
                            );
                            var link = $('<a href="javascript:void(0)" class="smarty-suggestion">' + suggAddr + "</a>");
            ...
            let autoCompleteParms = {
                ...
                prefer_cities: config.prefer_cities,
                prefer_states: config.prefer_states,
                prefer_zip_codes: config.prefer_zip_codes,
        ...
        function useAutocompleteSuggestion(addr, suggestion, containerUi) {
            if (config.debug)
                console.log(
                    "DEBUG: useAutocompleteSuggestion(addr, suggestion, containerUi)",
                    addr,
                    suggestion,
                    containerUi
                );
            ...
            if (addr.isFreeform()) {
                ...
                if (domfields.address1) $(domfields.address1).val(suggestion.street_line).change();
                ...
                if (domfields.administrative_area) {
                    if (domfields.administrative_area.options) {
                        // Checks for dropdown
                        for (var i = 0; i < domfields.administrative_area.options.length; i++) {
                            ...
                            if (
                                domfields.administrative_area.options[i].text.toUpperCase() === suggestion.state ||
                                allStatesByName[domfields.administrative_area.options[i].text.toUpperCase()] ===
                                    suggestion.state
                            ) {
        ...
        function filterDomElement(domElement, names, labels) {
            ...
            for (var i = 0; i < names.length; i++)
                if (name.indexOf(names[i]) > -1 || id.indexOf(names[i]) > -1) return true;
            ...
            for (var i = 0; i < domElement.labels.length; i++) {
                ...
                for (var j = 0; j < labels.length; j++)
                    if ($(domElement.labels[i]).text().toLowerCase().indexOf(labels[j]) > -1) return true;
            }

            // Still not found? Then look in "placeholder" or "title"...
            for (var i = 0; i < labels.length; i++)
                if (placeholder.indexOf(labels[i]) > -1 || title.indexOf(labels[i]) > -1) return true;
        ...
        this.hideAutocomplete = function (addressID) {
            $(".smarty-autocomplete.smarty-addr-" + addressID)
                .closest(".smarty-ui")
                .hide();
        };
        ...
        this.clean = function () {
            ...
            if (config.debug) console.log("Cleaning up old form map data and bindings...");
            ...
            for (var i = 0; i < forms.length; i++) {
                ...
                for (var j = 0; j < forms[i].addresses.length; j++) {
                    ...
                    if (doms.address1) $(doms.address1).off("keyup").off("keydown").off("blur");
                    else if (doms.freeform) $(doms.freeform).off("keyup").off("keydown").off("blur");
                }
            ...
            $(".smarty-ui")
                .off("click", ".smarty-suggestion")
                .off("mouseover", ".smarty-suggestion")
                .off("mouseleave", ".smarty-suggestion")
                .remove();
            ...
            if (config.debug) console.log("Done cleaning up; ready for new mapping.");
        };

        function disableBrowserAutofill(address) {
            //Does not disable autofill if config.max_results is disabled
        ...
        function addDefaultToStateDropdown(dom) {
            if (dom.getElementsByTagName("option").length > 0) {
                if (
                    arrayContains(stateNames, dom.getElementsByTagName("option")[0].text.toUpperCase()) ||
        ...
        this.mapFields = function (map) {
            ...
            if (config.debug) console.log("Manually mapping fields given this data:", map);
            ...
            for (var addrIdx in map) {
                if (map.hasOwnProperty(addrIdx)) {
                    ...
                    if (!address.country && config.target.indexOf("US") < 0) continue;
                        ...
                        if (address.hasOwnProperty(fieldType)) {
                            ...
                            if (fieldType != "id") {
                                if (!arrayContains(acceptableFields, fieldType)) {
                                    ...
                                    if (config.debug)
                                        console.log(
                                            "NOTICE: Field named " + fieldType + " is not allowed. Skipping..."
                                        );
                                    delete address[fieldType];
                                    continue;
                                }
                                ...
                                if (matched.length == 0) {
                                    ...
                                    if (config.debug)
                                        console.log(
                                            "NOTICE: No matches found for selector " +
                                                address[fieldType] +
                                                ". Skipping..."
                                        );
                                    delete address[fieldType];
                                } else if (matched.parents("form").length == 0) {
                                    // We should only map elements inside a <form> tag; otherwise we can't turn on submit handlers later
                                    if (config.debug)
                                        console.log(
                                            'NOTICE: Element with selector "' +
                                                address[fieldType] +
                                                '" is not inside a <form> tag. Skipping...'
                                        );
                                    delete address[fieldType];
                                } else address[fieldType] = matched[0];
                            }
                        }
                    }
                    if (config.target.indexOf("INTERNATIONAL") >= 0) {
                        if (
                            !(
                                (address.country && address.freeform) ||
                                (address.country && address.address1 && address.postal_code) ||
                                (address.country && address.address1 && address.locality && address.administrative_area)
                            )
                        ) {
                            if (config.debug)
                                console.log(
                                    "NOTICE: Address map (index " +
                                        addrIdx +
                                        ") was not mapped to a complete street address. Skipping..."
                                );
                            continue;
                        }
                    } else {
                        if (
                            !(
                                address.freeform ||
                                (address.address1 && address.postal_code) ||
                                (address.address1 && address.locality && address.administrative_area)
                            )
                        ) {
                            if (config.debug)
                                console.log(
                    ...
                    if (config.debug)
                        console.log(
                            "Finished mapping address with ID: " + form.addresses[form.addresses.length - 1].id()
                        );
                }
            }
        ...
        this.showLoader = function (addr) {
            ...
            loaderUI
                .css("top", lastFieldCorners.top + lastFieldCorners.height / 2 - loaderHeight / 2 + "px")
        ...
        this.markAsValid = function (addr) {
            ...
            domTag
                .removeClass("smarty-tag-grayed")
                .addClass("smarty-tag-green")
                .attr("title", "Address verified! Click to undo.");
            $(".smarty-tag-text", domTag)
                .text("Verified")
                .hover(
                    function () {
                        $(this).text("Undo");
        ...
        this.unmarkAsValid = function (addr) {
            ...
            domTag
                .removeClass("smarty-tag-green")
                .addClass("smarty-tag-grayed")
                .attr("title", "Address not verified. Click to verify.");
        ...
        this.showAmbiguous = function (data) {
            ...
            var html =
                ...
                corners.width +
                'px;">' +
                '<div class="smarty-popup-header smarty-popup-ambiguous-header">' +
                config.ambiguousMessage +
                "</div>" +
                '<div class="smarty-popup-typed-address">' +
                addr.toString() +
                "</div>" +
            ...
            if (addr.isDomestic()) {
                for (var i = 0; i < response.raw.length; i++) {
                    ...
                    html +=
                        '<a href="javascript:void(0)" class="smarty-choice" data-index="' +
                        i +
                        '">' +
                        line1 +
                        "<br>" +
                ...
                for (var i = 0; i < numCandidates; i++) {
                    ...
                    html += '<a href="javascript:void(0)" class="smarty-choice" data-index="' + i + '">' + ambigAddr + "</a>";
                }
            }

            html += '</div><div class="smarty-choice-alt">';
            html +=
                '<a href="javascript:void(0)" class="smarty-choice smarty-choice-abort smarty-abort">' +
                config.changeMessage +
                "</a>";
            if (!config.enforceVerification) {
                html +=
                    '<a href="javascript:void(0)" class="smarty-choice smarty-choice-override">' +
                    config.certifyMessage +
                    "</a>";
            }
            html += "</div></div></div>";
            ...
            if (
                $(document).scrollTop() > corners.top - 100 ||
            ...
            $("body").on("click", data.selectors.useOriginal, data, function (e) {
                $(this)
                    .parents(".smarty-popup")
                    .slideUp(defaults.speed, function () {
                        $(this).parent(".smarty-ui").remove();
                        $(this).remove();
                    });
        ...
        this.showInvalid = function (data) {
            ...
            var html =
                ...
                corners.width +
                'px;">' +
                '<div class="smarty-popup-header smarty-popup-invalid-header">' +
        ...
        this.showInvalidCountry = function (data) {
            ...
            var html =
                ...
                'px;">' +
                '<div class="smarty-popup-header smarty-popup-invalid-header">' +
                config.invalidCountryMessage +
                "</div>" +
                '<div class="smarty-popup-typed-address">' +
                addr.toString() +
                "</div>" +
                '<div class="smarty-choice-alt"><a href="javascript:void(0)" class="smarty-choice smarty-choice-abort smarty-abort">' +
                config.changeMessage +
                "</a></div>";
            if (!config.enforceVerification) {
                html +=
        ...
        this.showMissingSecondary = function (data) {
            ...
            var html =
                ...
                corners.width +
                'px;">' +
                '<div class="smarty-popup-header smarty-popup-missing-secondary-header">' +
                config.missingSecondaryMessage +
                "</div>" +
                '<div class="smarty-popup-typed-address">' +
                addr.toString() +
                "</div>" +
                '<form class="smarty-popup-secondary-number-form">' +
                '<input id="smarty-popup-secondary-number-input-box" class="smarty-addr-' +
                addr.id() +
                '" type="text" name="secondarynumber" placeholder="Enter number here"><br>' +
                '<input id="smarty-popup-secondary-number-form-submit-button" class="smarty-addr-' +
                addr.id() +
                ...
                'class="smarty-choice smarty-choice-abort smarty-abort">' +
                config.changeMessage +
                "</a>";
            if (!config.enforceVerification) {
                html +=
            ...
            $("body").on("click", data.selectors.submit, data, function (e) {
                e.data.address.secondary = $(
                    "#smarty-popup-secondary-number-input-box.smarty-addr-" + e.data.address.id()
                ).val();
        ...
        this.showMissingInput = function (data) {
            ...
            var html =
                ...
                corners.width +
                'px;">' +
                '<div class="smarty-popup-header smarty-popup-missing-input-header">' +
                config.missingInputMessage +
                "</div>" +
                '<div class="smarty-popup-typed-address">' +
                addr.toString() +
                "</div>" +
                '<div class="smarty-choice-alt"><a href="javascript:void(0)" ' +
                'class="smarty-choice smarty-choice-abort smarty-abort">' +
                config.changeMessage +
                "</a>";
            if (!config.enforceVerification) {
    ...
    function Address(domMap, formObj, addressID, match) {
        ...
        var doSet = function (key, value, updateDomElement, keepState, sourceEvent, suppressAutoVerify) {
			if(config.debug) console.log("DEBUG: ADDRESS.doSet(key, value, updateDomElement, keepState, sourceEvent, suppressAutoVerify", key, value, updateDomElement, keepState, sourceEvent, suppressAutoVerify);
            if (!arrayContains(acceptableFields, key) || !domMap[key]) {
                // Skip "id" and other unacceptable fields, also skip fields that weren't included in mapping
                return false;
            }
            ...
            if (
                typeof fields[key].dom !== "undefined" &&
                fields[key].dom.tagName === "SELECT" &&
                key !== "administrative_area" &&
                fields[key].dom.selectedIndex >= 0
            ) {
                        ...
                        .each(function () {
                            if ($(this).text().toUpperCase() === value) {
                                ...
                                for (var stateName in allStatesByName) {
                                    if (
                                        allStatesByName[stateName] === value &&
                                        stateName === $(this).text().toUpperCase()
                                    ) {
                                        selectedVal = $(this).val();
                                        return false;
                                    }
            ...
            if (differentVal && !keepState) {
                ...
                if (config.target.indexOf("US") >= 0 && config.target.indexOf("INTERNATIONAL") < 0) {
                    if (self.isDomestic()) {
                        if (uiTag && !uiTag.is(":visible")) uiTag.show(); // Show checkmark tag if address is in US
                        self.unaccept();
                        trigger("AddressChanged", eventMeta);
                    } else {
                        if (uiTag && uiTag.is(":visible")) uiTag.hide(); // Hide checkmark tag if address is non-US
                        self.accept(
                            {
                                address: self,
                            },
        ...
        this.load = function (domMap, addressID) {
            fields = {};
            id = addressID ? addressID.replace(/[^a-z0-9_\-]/gi, "") : randomInt(1, 99999); // Strips non-selector-friendly characters

            if (typeof domMap === "object") {
                // can be an actual map to DOM elements or just field/value data
                ...
                for (var prop in domMap) {
                    if (domMap.hasOwnProperty(prop)) {
                        if (!arrayContains(acceptableFields, prop))
                            // Skip "id" and any other unacceptable field
                            continue;

                        if (
                            typeof domMap[prop] == "object" &&
                            domMap[prop].getBoundingClientRect().top > this.lastField.getBoundingClientRect().top
                        ) {
                        ...
                        if (isData)
                            // Didn't match an HTML element, so treat it as an address string ("street1" data) instead
                            val = domMap[prop] || "";
                        else val = elem.val() || "";
        ...
        this.replaceWith = function (resp, updateDomElement, e) {
            ...
            if (typeof resp === "array" && resp.length > 0) resp = resp[0];

            // Sent via US api
            if (typeof resp.candidate_index != "undefined") {
                if (self.isFreeform()) {
                    var singleLineAddr =
                        (resp.addressee ? resp.addressee + ", " : "") +
                    ...
                    if (resp.delivery_line_2)
                        self.set("address2", resp.delivery_line_2, updateDomElement, true, e, false);
                    // Rarely used; must otherwise be blank.
                    else self.set("address2", "", updateDomElement, true, e, false);
                    self.set("locality", resp.components.city_name, updateDomElement, true, e, false);
                    self.set(
                        "administrative_area",
                        resp.components.state_abbreviation,
                        updateDomElement,
                        true,
                        e,
                        false
                    ...
                    self.set(
                        "postal_code",
                        resp.components.zipcode + "-" + resp.components.plus4_code,
                        updateDomElement,
                        true,
                        e,
                        false
                ...
                if (self.isFreeform()) {
                    ...
                    self.set(
                        "administrative_area",
                        resp.components.administrative_area,
                        updateDomElement,
                        true,
                        e,
                        false
        ...
        var removeComponentsFromAddressLines = function (resp) {
            if (resp.metadata.hasOwnProperty("address_format")) {
                ...
                for (var addressLineNumber in addressFormatLines) {
                    var componentsToRemove = ["locality", "administrative_area", "postal_code", "country"];
            ...
            if (!lastField) {
                for (var prop in fields) {
                    if (fields.hasOwnProperty(prop)) {
                        if (!fields[prop].dom || !$(fields[prop].dom).is(":visible")) continue;
        ...
        this.verify = function (invoke, invokeFn) {
            ...
            var credentials = config.token
                ? "auth-id=" + encodeURIComponent(config.key) + "&auth-token=" + encodeURIComponent(config.token)
        ...
        this.enoughInput = function () {
            ...
            if (fields.administrative_area) {
                ...
                if (
                    fields.administrative_area.dom !== undefined &&
                    fields.administrative_area.dom.length !== undefined
                ) {
                    if (fields.administrative_area.dom.selectedIndex < 1) stateText = "";
                    else
                        stateText =
                            fields.administrative_area.dom.options[fields.administrative_area.dom.selectedIndex].text;
                }
            }
            ...
            return (
                postalCodeIsPresentAndHasValue ||
                (localityIsPresentAndHasValue && administrativeAreaIsPresentAndHasValue)
            );
        };
        ...
        this.toRequestIntl = function () {
            var obj = {};
            if (
                fields.hasOwnProperty("freeform") &&
        ...
        this.toRequestUS = function () {
            ...
            } else if (fields.address1 && fields.address1.value) {
                // Covers a special case where the user calls .verify on the instance
            ...
            if (fields.administrative_area && fields.administrative_area.dom.value) {
                if (
                    fields.administrative_area.dom.tagName === "SELECT" &&
                    fields.administrative_area.dom.selectedIndex >= 0
                ) {
        ...
        this.toString = function () {
            if (fields.freeform) {
                return (
                    (fields.freeform ? fields.freeform.value + " " : "") + (fields.country ? fields.country.value : "")
                );
            } else {
                var addrString =
                    (fields.address1 ? fields.address1.value + " " : "") +
                    (fields.address2 ? fields.address2.value + " " : "") +
                    (fields.address3 ? fields.address3.value + " " : "") +
                    (fields.address4 ? fields.address4.value + " " : "") +
        ...
        this.accept = function (data, showValid) {
            ...
            if (showValid)
                // If user chooses original input or the request timed out, the address wasn't "verified"
                ui.markAsValid(self);
        ...
        this.isDomestic = function () {
            ...
            if (fields.country && fields.country.dom) {
                ...
                if (selectedOption.length > 0 && selectedOption.index() > 0) countryValue = selectedOption.text();
            }
        ...
        this.autocompleteVisible = function () {
            return config.ui && config.max_results && $(".smarty-autocomplete.smarty-addr-" + self.id()).is(":visible");
        };
    ...
    function Form(domElement) {
        ...
        this.activeAddressesNotAccepted = function () {
            ...
            for (var i = 0; i < this.addresses.length; i++) {
                ...
                if (addr.status() != "accepted" && addr.active) addrs.push(addr);
            }
            return addrs;
        };
    ...
    function Response(json) {
        ...
        this.isMissingSecondary = function (idx) {
            ...
            checkBounds(idx);
            return (
                (this.raw[idx].analysis.dpv_footnotes && this.raw[idx].analysis.dpv_footnotes.indexOf("N1") > -1) ||
    ...
    function actionsOnChange(e) {
        ...
        if (
            clickyIsBoldAndNotActive() ||
            clickyIsNotBoldAndNotActive() ||
            clickyAndTypeyAreNullAndAutocompleteVisible()
        ) {
    ...
    var submitForm = function (invokeOn, invokeFunction) {
        if (invokeOn && typeof invokeOn !== "function" && invokeFunction) {
            ...
            } else if (invokeFunction == "submit") $(invokeOn).submit(); // For submit(), we have to use jQuery's, so that all its submit handlers fire.
        }
    };
    ...
    function addPreconnectLinksToHead() {
        ...
        var usAutocompleteApi = "//us-autocomplete-pro.api.smartystreets.com";
        ...
        for (var i = 0; i < endpoints.length; i++) {
            ...
            $("head")
                ...
                .append('<link rel="dns-prefetch" ' + href);
        }
    }
