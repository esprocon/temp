...
(function ($, window, document) {
  ...
  var defaults = {
    ...
    missingSecondaryMessage:
      "You forgot your apt/suite number or entered an unknown apt/suite number", // Message when address is missing a secondary number
  ...
  $.fn.LiveAddress = function (arg) {
    ...
    if (arg.debug)
      console.log(
        "LiveAddress API jQuery Plugin version " + version + " (Debug mode)"
      );
    ...
    config.include_only_cities = typeof config.include_only_cities === "undefined" ? "" : config.include_only_cities;
    ...
    config.include_only_states = typeof config.include_only_states === "undefined" ? "" : config.include_only_states;
	...
	Example: 90210;06504*/
    config.include_only_zip_codes = typeof config.include_only_zip_codes === "undefined" ? "" : config.include_only_zip_codes;
    ...
    config.prefer_geolocation = typeof config.prefer_geolocation === "undefined" ? "city" : config.prefer_geolocation;
    ...
    instance = {
      events: {
        ...
        AddressChanged: function (event, data) {
		  ...
		  */
          ...
          if (
            ...
            (data.address.verifyCount == 0 ||
              data.address.isFreeform() ||
              data.address.usedAutocomplete) &&
        ...
        VerificationInvoked: function (event, data) {
          ...
          if (
            !data.address ||
            (data.address && data.address.form && data.address.form.processing)
          ) {
            if (config.debug)
              console.log(
            ...
            data.address.status() == "accepted" && !data.verifyAccepted
          ) {
            if (config.debug)
              console.log(
    ...
    // Map the fields
    if (
      config.target.indexOf("US") >= 0 ||
      config.target.indexOf("INTERNATIONAL") >= 0
    ) {
      instance.mapFields();
    } else if (config.debug) {
      console.log(
        'Proper target not set in configuration. Please use "US" or "INTERNATIONAL".'
      );
    }
  ...
  function UI() {
          ...
          $(window).on(
            ...
            function (e) {
              ...
              if (config.max_results) {
                ...
                var containerUi = $(
                  ".smarty-autocomplete.smarty-addr-" + addr.id()
                ).closest(".smarty-ui");
            ...
            for (var j = 0; j < f.addresses.length; j++) {
              ...
              if (mainInput !== "") {
                ...
                containerUi.css({
                  ...
                  top:
                    strField.offset().top + strField.outerHeight(false) + "px",
                });
                ...
                containerUi.on("mouseover", ".smarty-suggestion", function () {
                  $(".smarty-active-suggestion").removeClass(
                    "smarty-active-suggestion"
                  );
        ...
        callback: function (counter, json) {
          ...
          if (!json.suggestions || json.suggestions.length == 0) {
            data.suggContainer.html(
              '<div class="smarty-no-suggestions">' +
                config.noSuggestionsMessage +
                "</div>"
            );
            return;
          }
          ...
          if (config.waitForStreet && filtering == false) {
            ...
            data.suggContainer.html(
              '<div class="smarty-no-suggestions">Type more for ' +
                message +
                " suggestions</div>"
            );
          } else {
    ...
    this.showAutocomplete = function (event, data) {
      if (autocplRequests[data.autocplrequest.number])
        autocplRequests[data.autocplrequest.number].callback(
          data.autocplrequest.number,
          data.json
        );
    };
      ...
      if (addr.isFreeform()) {
        ...
        if (domfields.administrative_area) {
          if (domfields.administrative_area.options) {
            ...
            for (
              var i = 0;
              i < domfields.administrative_area.options.length;
              i++
            ) {
              // Checks for abbreviation match and maps full state name to abbreviation
              if (
                domfields.administrative_area.options[i].text.toUpperCase() ===
                  suggestion.state ||
                allStatesByName[
                  domfields.administrative_area.options[i].text.toUpperCase()
                ] === suggestion.state
    ...
    function filterDomElement(domElement, names, labels) {
      ...
      if (!("labels" in domElement)) {
        var lbl =
          $('label[for="' + selectorSafeID + '"]')[0] ||
          $(domElement).parents("label")[0];
      ...
      for (var i = 0; i < domElement.labels.length; i++) {
        ...
        for (var j = 0; j < labels.length; j++)
          if (
            $(domElement.labels[i]).text().toLowerCase().indexOf(labels[j]) > -1
          )
            return true;
      }
      ...
      for (var i = 0; i < labels.length; i++)
        if (
          placeholder.indexOf(labels[i]) > -1 ||
          title.indexOf(labels[i]) > -1
        )
          return true;
    ...
    function turnOffAllClicks(selectors) {
      if (Array.isArray(selectors) || typeof selectors == "object") {
        ...
        alert(
          "ERROR: Not an array, string, or object passed in to turn off all clicks"
        );
      }
    }
        ...
        for (var j = 0; j < forms[i].addresses.length; j++) {
          ...
          for (var prop in doms) {
            if (doms.hasOwnProperty(prop)) {
              if (config.debug) {
                $(doms[prop])
                  .removeClass("smarty-debug-input")
                  .attr("placeholder", "");
    ...
    function addDefaultToStateDropdown(dom) {
      if (dom.getElementsByTagName("option").length > 0) {
        if (
          arrayContains(
            stateNames,
            dom.getElementsByTagName("option")[0].text.toUpperCase()
          ) ||
          arrayContains(
            stateAbbreviations,
            dom.getElementsByTagName("option")[0].text.toUpperCase()
    ...
    this.mapFields = function (map) {
      ...
      for (var addrIdx in map) {
        if (map.hasOwnProperty(addrIdx)) {
          ...
          if (config.debug)
            console.log(
              "Finished mapping address with ID: " +
                form.addresses[form.addresses.length - 1].id()
            );
        }
      }
    ...
    this.showLoader = function (addr) {	  
      ...
      loaderUI
        .css(
          "top",
          lastFieldCorners.top +
            lastFieldCorners.height / 2 -
            loaderHeight / 2 +
            "px"
        )
    ...
    this.showAmbiguous = function (data) {	  	  
      ...
      if (addr.isDomestic()) {
        for (var i = 0; i < response.raw.length; i++) {
          var line1 = response.raw[i].delivery_line_1,
            ...
            zip =
              response.raw[i].components.zipcode +
              "-" +
              response.raw[i].components.plus4_code;
          html +=
            '<a href="javascript:" class="smarty-choice" data-index="' +
            i +
            '">' +
            line1 +
            "<br>" +
      ...
      html +=
        '<a href="javascript:" class="smarty-choice smarty-choice-abort smarty-abort">' +
        config.changeMessage +
        "</a>";
      if (!config.enforceVerification) {
        html +=
          '<a href="javascript:" class="smarty-choice smarty-choice-override">' +
      ...
      if (
        ...
        $("html, body")
          ...
          .animate(
            {
              scrollTop:
                $(".smarty-popup.smarty-addr-" + addr.id()).offset().top - 100,
            },
            500
          );
      }
      ...
      data.selectors = {
        goodAddr:
          ".smarty-popup.smarty-addr-" +
          addr.id() +
          " .smarty-choice-list .smarty-choice",
        useOriginal:
          ".smarty-popup.smarty-addr-" + addr.id() + " .smarty-choice-override",
      ...
      $("body").on("click", data.selectors.goodAddr, data, function (e) {
        $(".smarty-popup.smarty-addr-" + addr.id()).slideUp(
          defaults.speed,
          function () {
            $(this).parent(".smarty-ui").remove();
            $(this).remove();
    ...
    this.showMissingSecondary = function (data) {  	  
      ...
      data.selectors = {
        ...
        abort: ".smarty-popup.smarty-addr-" + addr.id() + " .smarty-abort",
        submit:
          "#smarty-popup-secondary-number-form-submit-button.smarty-addr-" +
          addr.id(),
      };
      ...
      $("body").on("click", data.selectors.submit, data, function (e) {
        e.data.address.secondary = $(
          "#smarty-popup-secondary-number-input-box.smarty-addr-" +
            e.data.address.id()
        ).val();
  ...
  function Address(domMap, formObj, addressID, match) {
    ...
    var doSet = function (
      ...
      if (
        ...
        value = fields[key].dom[fields[key].dom.selectedIndex].text.replace(
          /<|>/g,
          ""
        );
      } else {
      ...
      if (differentVal && !keepState) {
        ...
        var uiTag =
          config.ui && config.smartyTag
            ? $(".smarty-ui .smarty-tag.smarty-addr-" + id)
            : undefined;
        if (
          config.target.indexOf("US") >= 0 &&
          config.target.indexOf("INTERNATIONAL") < 0
    ...
    this.load = function (domMap, addressID) {
      fields = {};
      id = addressID
        ? addressID.replace(/[^a-z0-9_\-]/gi, "")
        : randomInt(1, 99999); // Strips non-selector-friendly characters
      ...
      if (typeof domMap === "object") {
        ...
        this.lastField =
          domMap[Object.keys(domMap)[Object.keys(domMap).length - 1]];
        ...
        for (var prop in domMap) {
          if (domMap.hasOwnProperty(prop)) {
            ...
            if (
              ...
              domMap[prop].getBoundingClientRect().top >
                this.lastField.getBoundingClientRect().top
            ) {
    ...
    this.replaceWith = function (resp, updateDomElement, e) {  	  
      ...
      if (typeof resp.candidate_index != "undefined") {
        ...
        if (self.isFreeform()) {
          ...
          var countryLine = resp.components.country_iso_3
            ? resp.components.country_iso_3
            : "";
          self.set(
            "freeform",
            singleLineAddr,
          ...
          if (resp.components.postal_code_short) {
            ...
            if (resp.components.postal_code_extra)
              fullPostalCode =
                fullPostalCode + "-" + resp.components.postal_code_extra;
            self.set(
              "postal_code",
              fullPostalCode,
              updateDomElement,
    ...
    var removeComponentFromAddressLine = function (
      addressComponent,
      componentName,
    ...
    var emptyLastNonEmptyAddressLine = function (resp) {
      for (var lineNumber = 12; lineNumber >= 1; lineNumber--) {
        ...
        if (
          resp.hasOwnProperty(addressLineComponent) &&
      ...
      if (resp.metadata.hasOwnProperty("address_format")) {
        ...
        for (var addressLineNumber in addressFormatLines) {
          ...
          componentsToRemove.map(function (componentName) {
            removeComponentFromAddressLine(
              addressComponent,
              componentName,
              addressLineComponent,
              resp
    ...
    var addAddressLine = function (fullLine, addressLine, nextAddressLine) {
      if (
        typeof addressLine !== "undefined" &&
        typeof nextAddressLine !== "undefined"
      ) {
        if (fullLine !== "") {
          fullLine += ", ";
        }
      ...
      if (!lastField) {
        for (var prop in fields) {
          if (fields.hasOwnProperty(prop)) {
            ...
            corners.top = !corners.top
              ? offset.top
              : Math.min(corners.top, offset.top);
            corners.left = !corners.left
              ? offset.left
              : Math.min(corners.left, offset.left);
            corners.right = !corners.right
              ? offset.right
              : Math.max(corners.right, offset.right);
            corners.bottom = !corners.bottom
              ? offset.bottom
              : Math.max(corners.bottom, offset.bottom);
          }
        }
      } else {
    ...
    this.enoughInput = function () {	  
      ...
      if (fields.administrative_area) {
        ...
        if (
          ...
          if (fields.administrative_area.dom.selectedIndex < 1) stateText = "";
            ...
            stateText =
              fields.administrative_area.dom.options[
                fields.administrative_area.dom.selectedIndex
              ].text;
        }
      }
      ...
      var postalCodeIsPresentAndHasValue = !!(
        fields.postal_code && fields.postal_code.value
      );
      var administrativeAreaIsPresentAndHasValue = !!(
        fields.administrative_area && stateText.length > 0
      );
      var localityIsPresentAndHasValue = !!(
    ...
    this.toRequestUS = function () {
      ...
      if (fields.locality && fields.locality.dom) {
        if (
          fields.locality.dom.tagName === "SELECT" &&
          fields.locality.dom.selectedIndex >= 0
        ) {
          obj.city =
            fields.locality.dom[fields.locality.dom.selectedIndex].text;
        } else {
      ...
      if (fields.administrative_area && fields.administrative_area.dom.value) {
        if (
          ...
          obj.state =
            fields.administrative_area.dom[
              fields.administrative_area.dom.selectedIndex
            ].text;
        } else {
    ...
    this.toString = function () {
      if (fields.freeform) {
        return (
          ...
          (fields.country ? fields.country.value : "")
        );
      } else {
        var addrString =
    ...
    this.undo = function (updateDomElement) {
      updateDomElement =
        typeof updateDomElement === "undefined" ? true : updateDomElement;
      for (var key in fields) {
        if (fields.hasOwnProperty(key)) {
          this.set(
            key,
    ...
    this.autocompleteVisible = function () {
      return (
        config.ui &&
        config.max_results &&
        $(".smarty-autocomplete.smarty-addr-" + self.id()).is(":visible")
      );
    };
  ...
  function Response(json) {
    ...
    var checkBounds = function (idx) {
      ...
      if (idx >= json.length || idx < 0) {
        if (json.length == 0)
          throw new Error(
            "Candidate index is out of bounds (no candidates returned; requested " +
              idx +
              ")"
          );
        else
    ...
    this.isMissingSecondary = function (idx) {
      ...
      checkBounds(idx);
      return (
        (this.raw[idx].analysis.dpv_footnotes &&
          this.raw[idx].analysis.dpv_footnotes.indexOf("N1") > -1) ||
        (this.raw[idx].analysis.dpv_footnotes &&
          this.raw[idx].analysis.dpv_footnotes.indexOf("R1") > -1) ||
        (this.raw[idx].analysis.footnotes &&
  ...
  function actionsOnChange(e) {
    ...
    function clickyIsBoldAndNotActive() {
      return (
        !clickyIsNull() &&
        clickyIsBold() &&
        !elementIsActiveSuggestion(clicky[0].parentElement)
      );
    }
    ...
    function clickyIsNotBoldAndNotActive() {
      return (
        !clickyIsNull() &&
        !clickyIsBold() &&
        !elementIsActiveSuggestion(clicky[0])
      );
    }

    function clickyAndTypeyAreNullAndAutocompleteVisible() {
      return (
        clickyIsNull() && typeyIsNull() && e.data.address.autocompleteVisible()
      );
    }
